...
export class ProductsController {
  ...
  async index(request: Request, response: Response) {
    ...
    const is_new =
      request.query.is_new === undefined
        ? undefined
        : request.query.is_new === "true";
    const accept_trade =
      request.query.accept_trade === undefined
        ? undefined
        : request.query.accept_trade === "true";
    const query =
      typeof request.query.query === "string" ? request.query.query : undefined;
    const payment_methods =
      request.query.payment_methods === undefined
        ? undefined
  ...
  async create(request: Request, response: Response) {
    ...
    if (typeof is_new === "string" ? !is_new : typeof is_new === undefined) {
      throw new AppError(
        "A condição (novo ou usado) do produto é obrigatória."
      );
    }
    ...
    if (
      typeof accept_trade === "string"
        ? !accept_trade
        : typeof accept_trade === undefined
    ) {
  ...
  async update(request: Request, response: Response) {
    ...
    if (product.user_id !== userId) {
      throw new AppError(
        "O usuário não tem permissão para atualizar o produto",
        401
      );
    }
    ...
    if (Array.isArray(payment_methods)) {
      if (payment_methods.length === 0) {
        throw new AppError(
          "O produto precisa ter pelo menos 1 método de pagamento."
        );
      }
      ...
      oldPaymentsMethods = product.payment_methods
        .filter(
          (payment_method) => !payment_methods.includes(payment_method.key)
        )
        .map((payment_method) => payment_method.key);
      newPaymentsMethods = payment_methods.filter(
        (payment_method) => !product.payment_methods.includes(payment_method)
      );
    }
